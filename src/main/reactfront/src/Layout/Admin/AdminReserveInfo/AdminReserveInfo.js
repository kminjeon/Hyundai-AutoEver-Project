...
import BookItem_ReserveInfo from '../ADMIN_BookItem/BookItem_ReserveInfo';

const AdminReserveInfo = () => {
    ...
    useEffect(() => {
      ...
      const getRentPage = async () => {
        ...
        try {
            const response = await axios.get(`/api/admin/reserve`, {
        params : {
            page : page,
      ...
      <div>
        ...
        <ol className='numbered-list'>
          {reserveList && reserveList.map((reserve, index) => {
            return <BookItem_ReserveInfo key={reserve.reserveId} book ={reserve} index={(page * 10 ) + index + 1} /> 
          })}
        </ol>
